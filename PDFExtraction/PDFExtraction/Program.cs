...
namespace SamplePDF
{
    class Program : PdfFile
    {
        ...
        public static void ExtractXml(string pdfPath)
        {
            ...
            //Removed trial version message from the file and convernt it to json

            List<string> lines = File.ReadAllLines(xmlPath).ToList();
            System.IO.File.WriteAllLines(xmlPath, lines.GetRange(0, lines.Count - 8));
            File.AppendAllText(xmlPath, "</document>" + Environment.NewLine);
